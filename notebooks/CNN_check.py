...
def run_training():
    ...
    for i in range(2):
        data_sets = input_data.read_data_sets(os.path.join(DATA_PATH, 'val_'+str(IMAGE_SIZE)+'_'+str(i+1)+'.mat'),'test')
        if(i==0):
            Test_data = data_sets
            continue
        else:
    ...
    # Generate placeholders for the images and labels.
        ...
        loss = IndianPinesTF.loss(logits, labels_placeholder)

        # Add to the Graph the Ops that calculate and apply gradients.
        train_op = IndianPinesTF.training(loss, FLAGS.learning_rate)
    ...
    #             summary_writer.flush()
            ...
            if (step + 1) % 1000 == 0 or (step + 1) == FLAGS.max_steps:
                saver.save(sess, 'model-spatial-'+str(IMAGE_SIZE)+'X'+str(IMAGE_SIZE)+'.ckpt', global_step=step)
